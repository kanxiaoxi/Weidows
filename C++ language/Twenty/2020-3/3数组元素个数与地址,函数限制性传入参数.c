 ...
 * @LastEditors: Weidows
 * @LastEditTime: 2020-05-29 00:11:24
 * @FilePath: \demo\Notes\C\1\.3数组元素个数与地址,函数限制性传入参数.c
 */ 
#include<stdio.h>
int main(){
    int i=5;
    ...
    const int b[5] = {1, 2, 3, 4, 5};/*const定义数组只能一开始进行
    集合初始化,数组内元素个数和值之后都不可以改变,b[5]中的5也不可以用变量代替*/
    return 0;
}
void sum(const int a[], int length){
/*为了保护数组值(数组本身也是指针,进入函数是可能会被改变)
可以在声明函数原型时把参数加上const,不会改变原数组类型
只是在函数内部把数组看做const使用,但是const类型数组不能
作为int类型参数传入函数,以此类推...*/
